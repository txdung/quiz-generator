...
- **902 câu hỏi nhúng sẵn** trong file HTML
- **Dark theme** xanh xám tối — dịu mắt khi làm bài lâu

### 2. 📚 Dữ liệu câu hỏi (902 câu)
...
- **🔍 Tìm kiếm + Bộ lọc** — Search bar real-time (debounce 300ms), highlight kết quả, toggle selection per result, "Làm quiz từ kết quả"
- **⚠️ Báo đáp án sai** — Modal report trong review, lưu localStorage, corrections panel, Export/Import JSON
- **Collapsible Sheet Selection** — Mặc định thu gọn, click header toggle mở/đóng, animation smooth
- **Rebuild Pipeline** — `rebuild.py` (pandas column-name-based) + `corrections.json` → `all_questions.json`
...
| Sửa đáp án Kíp Trưởng #22 | rơle anten cổng COM | A → **C** |
| Sửa đáp án Kíp Trưởng #230 | Thẩm quyền ban hành quyết định | D → **B** |
| Collapsible sheet toggle không hoạt động | Inline style `!important` override CSS class | Dùng inline style hoàn toàn |
| VHF missing corrections | Rebuild script skip sheet | Fix rebuild.py FILES config |
...
├── all_questions.json           ← Dữ liệu 902 câu hỏi
...
### Variables
- `embeddedQuestions[]` — Mảng 902 câu hỏi (`mode: 'vhf'`, `'radar'`, `'kip_truong'`)
...
**Tổng kết:** Quiz Generator đã hoàn thành với 902 câu hỏi VHF + Radar + Kíp Trưởng, dark theme xanh xám tối dịu mắt, giao diện đồng bộ, bookmark, luyện tập câu sai, thống kê điểm số, PWA (cài đặt như app trên điện thoại), GitHub Pages hosting — chạy offline hoàn toàn. **Mới nhất:** 🔍 Search + filter, ⚠️ Báo đáp án sai (localStorage → Export/Import JSON), collapsible sheet selection, rebuild pipeline unified.
